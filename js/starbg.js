...
for(var i=0;i<nums;i++){
    var arrs=["t","f"];
    var r= Math.random()*1;  //鐢熸垚鏄熸槦鐨勫崐寰?
...
var redYlimt=-0.35*canvasElement.width*0.22;;
 function draw(){
   ...
   /*  colors.addColorStop(0,"#090723");
     ...
     //鐢诲渾 color r x y
    /* drawCircle("#f7f4d3",canvasElement.width*0.12,0.44*canvasElement.width,redYlimt,"#fcf6ac");                         //绾?
     drawCircle("#201620",canvasElement.width*0.025,0.95*canvasElement.width,0.32*canvasElement.height);       //鐏?
     drawCircle("#140c2a",canvasElement.width*0.01,0.75*canvasElement.width,0.22*canvasElement.height);        //绱?
     drawCircle("#090929",canvasElement.width*0.018,0.82*canvasElement.width,0.52*canvasElement.height);       //钃?*/
     if(redstart==true){
         redYlimt+=1;
     }
     if(redYlimt>yred){
     ...
     for(var i=0;i<starArr.length;i++){
         ...
         if(child.moves=='t'){
             child.initX-=Math.random()*1*0.12;  //鏄熸槦绉诲姩鐨勯€熷害
             if(child.initX<0){
                 child.initX=child.x;
             }
         }
  ...
  function drawCircle(color,R,x,y,color2){
    ...
    //浠ｇ爜鎬濊矾锛?
    //鏄熸槦 闅忔満浜х敓涓€瀹氭暟閲忕殑鏄熸槦瀵硅薄{鍒濆浣嶇疆:x,y;鏄熸槦鍗婂緞}
    //鏄熸槦闂儊涓庤繍鍔ㄧ殑鍘熺悊 姣忛殧涓€瀹氱鏁伴噸鏂扮粯鍒? 骞朵笖绉诲姩浣嶇疆 涓? 鏀瑰彉閫忔槑搴?  闂儊涓庣Щ鍔ㄥ彲浠ラ殢鏈烘垨鑰呬竴瀹氱殑瑙勫緥
    //
    //娴佺嚎  鍚屾祦鏄熷師鐞嗙被浼?  鏂滅嚎鏍规嵁瑙掑害涓庣涓€鐐? 姹傜浜岀偣
}